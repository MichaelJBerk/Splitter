...
//			searchPathDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first
        case .caches:
			searchPathDirectory = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first
		case .appSupport:
			
			try? FileManager.default.createDirectory(at: appSupport!, withIntermediateDirectories: true, attributes: nil)
			searchPathDirectory = appSupport
        }

		if let url = searchPathDirectory {//FileManager.default.urls(for: searchPathDirectory, in: .userDomainMask).first {
            return url
        } else {
