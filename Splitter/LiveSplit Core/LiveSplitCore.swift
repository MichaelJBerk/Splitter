...
public class SeparatorComponentRefMut: SeparatorComponentRef {
    /**
        Calculates the component's state based on the timer provided.
    */
    public func state(_ timer: TimerRef) -> SeparatorComponentState {
        assert(self.ptr != Optional.none)
        assert(timer.ptr != Optional.none)
        let result = SeparatorComponentState(ptr: LiveSplitCoreNative.SeparatorComponent_state(self.ptr, timer.ptr))
        return result
    }
    override init(ptr: UnsafeMutableRawPointer?) {
        super.init(ptr: ptr)
